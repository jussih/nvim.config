...
-- Native insert mode autocomplete
...
require('lazy').setup({
  ...
  { -- Fuzzy Finder (files, lsp, etc)
    ...
    config = function()
      ...
      -- Live grep (fzf-lua has no open-files-only grep, this searches the project)
      vim.keymap.set('n', '<leader>s/', fzf.live_grep, { desc = '[S]earch [/] Live Grep' })
  ...
  -- LSP Plugins
  {
    ...
    config = function()
      ...
      --  The Language servers must be separately installed
      --  See the lsp configs in https://github.com/neovim/nvim-lspconfig/tree/master/lsp
  ...
  { -- Autoformat
    ...
    keys = {
      {
        '<leader>f',
        function() require('conform').format { async = true, lsp_format = 'fallback', stop_after_first = true  } end,
        mode = '',
        desc = '[F]ormat buffer',
      },
    },
      ...
      statusline.setup {
        content = {
          ...
          active = function()
            ...
            local git           = MiniStatusline.section_git({ trunc_width = 150 })
